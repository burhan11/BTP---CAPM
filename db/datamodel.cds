...
using {burhan.common} from './commons';
...
context master {
    ...
    entity product {
        key NODE_KEY       : common.Guid           @title: '{i18n>PRODUCT_NODEKEY}';
    ...
    entity status {
        key STATUS_ID          : String(1)  @(title: '{i18n>OVERALL_STATUS}');
            STATUS_DESCRIPTION : String(10) @(title: '{i18n>OVERALL_STATUS}');
    }
}

context transaction {
    entity purchaseorder : common.Amount {
        key NODE_KEY         : UUID       @title: '{i18n>PO_NODEKEY}';
            PO_ID            : String(40) @title: '{i18n>PO_ID}';
            PARTNER_GUID     : Association to master.businesspartner;
            LIFECYCLE_STATUS : String(1);
            // OR @readonly
            OVERALL_STATUS   : Association to one master.status; //Create function OR default 'N'; 
            Items            : Composition of many poitems
                                   on Items.PARENT_KEY = $self;
    }

    entity poitems : common.Amount {
            @title: '{i18n>POITEM_NODEKEY}'
        key NODE_KEY     : UUID; //common.Guid;
